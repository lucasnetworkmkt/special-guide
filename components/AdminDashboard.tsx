...
import { fetchAnnouncements, createAnnouncement, toggleAnnouncement, DEFAULT_MENU_ITEMS } from '../services/supabase';
...
// Helper to generate SQL insert values
const generateMenuInserts = () => {
  const values = DEFAULT_MENU_ITEMS.map(item => 
    `('${item.id}', '${item.name.replace(/'/g, "''")}', '${item.description.replace(/'/g, "''")}', ${item.price}, '${item.category}', ${item.highlight}, '${item.image}')`
  ).join(',\n  ');
  return values;
};

const SQL_COMMAND = `-- Tabela de Reservas
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ reservations, menuItems, onUpdateStatus, onUpdateMenuPrice, onLogout, isDbConnected }) => {
  ...
  const handlePostAnnouncement = async () => {
    ...
    if (newItem) {
      // Just add new item, don't disable others in UI since backend supports multiple now
                    ...
                    <div className="flex flex-col items-end gap-2">
                       {editingPriceId === item.id ? (
                         ...
                         <div className="flex items-center gap-3">
                           <span className="font-bold text-emerald-400">R$ {parseFloat(item.price).toFixed(2)}</span>
                           <button 
